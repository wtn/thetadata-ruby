...
describe ThetaData::REST::Option do
  ...
  with ".snapshot_ohlc" do
    ...
    it "passes max_dte / strike_range / min_time through" do
      with_mock_connection(ohlc_response) do |conn|
        ThetaData::REST::Option.snapshot_ohlc(symbol: "AAPL", expiration: "2025-01-17", max_dte: 30, strike_range: 5, min_time: "10:00:00")
        params = conn.last_call[:request].params
        expect(params.max_dte).to be == 30
        expect(params.strike_range).to be == 5
  ...
  with ".snapshot_market_value" do
    let(:market_value_response) do
      {
        headers: %w[timestamp symbol expiration strike right market_bid market_ask market_price],
        rows: [[Time.new(2024, 12, 2), "AAPL", "2025-01-17", BigDecimal("150.00"), "CALL", BigDecimal("5.20"), BigDecimal("5.30"), BigDecimal("5.25")]],
      }
    end

    it "calls connection with correct method" do
      with_mock_connection(market_value_response) do |conn|
        ThetaData::REST::Option.snapshot_market_value(symbol: "AAPL", expiration: "2025-01-17", strike: 150, right: "CALL")
        expect(conn.last_call[:method]).to be == :GetOptionSnapshotMarketValue
      end
    end

    it "returns OptionSnapshotMarketValueRow Data objects" do
      with_mock_connection(market_value_response) do |conn|
        result = ThetaData::REST::Option.snapshot_market_value(symbol: "AAPL", expiration: "2025-01-17")
        expect(result).to be_a(Array)
        expect(result.first).to be_a(ThetaData::REST::OptionSnapshotMarketValueRow)
        expect(result.first.market_bid).to be == BigDecimal("5.20")
        expect(result.first.market_ask).to be == BigDecimal("5.30")
        expect(result.first.market_price).to be == BigDecimal("5.25")
      end
    end
  end
  ...
  with ".history_ohlc" do
    ...
    it "passes start_date/end_date through for multi-day range" do
      with_mock_connection(ohlc_response) do |conn|
        ThetaData::REST::Option.history_ohlc(symbol: "AAPL", expiration: "2025-01-17", start_date: Date.new(2024, 12, 1), end_date: Date.new(2024, 12, 31), interval: 60000, strike_range: 5)
        params = conn.last_call[:request].params
        expect(params.start_date).to be == "2024-12-01"
        expect(params.end_date).to be == "2024-12-31"
        expect(params.strike_range).to be == 5
      end
    end
  end
  ...
  with ".snapshot_greeks_implied_volatility" do
    ...
    it "passes greeks calculation kwargs through" do
      with_mock_connection(greeks_response) do |conn|
        ThetaData::REST::Option.snapshot_greeks_implied_volatility(
          symbol: "AAPL", expiration: "2025-01-17",
          annual_dividend: 0.005, rate_type: "SOFR", rate_value: 0.0425,
          stock_price: 155.0, version: "v3", max_dte: 30, strike_range: 5,
          min_time: "10:00:00", use_market_value: true,
        )
        params = conn.last_call[:request].params
        expect(params.annual_dividend).to be == 0.005
        expect(params.rate_type).to be == "SOFR"
        expect(params.rate_value).to be == 0.0425
        expect(params.stock_price).to be == 155.0
        expect(params.version).to be == "v3"
        expect(params.max_dte).to be == 30
        expect(params.strike_range).to be == 5
        expect(params.min_time).to be == "10:00:00.000"
        expect(params.use_market_value).to be == true
      end
    end
  end
  ...
  with "right validation" do
    ...
    it "normalizes :call symbol to \"CALL\"" do
      with_mock_connection(trade_response) do |conn|
        ThetaData::REST::Option.snapshot_trade(symbol: "AAPL", expiration: "2025-01-17", strike: 150, right: :call)
        expect(conn.last_call[:request].params.contract_spec.right).to be == "CALL"
      end
    end

    it "normalizes \"P\" shorthand to \"PUT\"" do
      with_mock_connection(trade_response) do |conn|
        ThetaData::REST::Option.snapshot_trade(symbol: "AAPL", expiration: "2025-01-17", strike: 150, right: "P")
        expect(conn.last_call[:request].params.contract_spec.right).to be == "PUT"
      end
    end

    it "leaves nil right unset" do
      with_mock_connection(trade_response) do |conn|
        ThetaData::REST::Option.snapshot_trade(symbol: "AAPL", expiration: "2025-01-17", strike: 150)
        # ContractSpec.right has the proto3 unset default for an optional string
        expect(conn.last_call[:request].params.contract_spec.right).to be == ""
      end
    end

    it "raises ArgumentError for an unrecognized right" do
      with_mock_connection(trade_response) do |conn|
        expect do
          ThetaData::REST::Option.snapshot_trade(symbol: "AAPL", expiration: "2025-01-17", strike: 150, right: "junk")
        end.to raise_exception(ArgumentError, message: be =~ /right must be one of/)
      end
    end
  end
end

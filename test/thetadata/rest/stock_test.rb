...
describe ThetaData::REST::Stock do
  ...
  with ".snapshot_ohlc" do
    ...
    it "passes min_time through to the request" do
      with_mock_connection(ohlc_response) do |conn|
        ThetaData::REST::Stock.snapshot_ohlc("AAPL", min_time: "10:00:00")
  ...
  with ".snapshot_market_value" do
    let(:market_value_response) do
      {
        headers: %w[timestamp symbol market_bid market_ask market_price],
        rows: [[Time.new(2024, 12, 2, 16, 0, 0), "AAPL", BigDecimal("154.45"), BigDecimal("154.55"), BigDecimal("154.50")]],
      }
    end

    it "calls connection with correct method" do
      with_mock_connection(market_value_response) do |conn|
        ThetaData::REST::Stock.snapshot_market_value("AAPL")
        expect(conn.last_call[:method]).to be == :GetStockSnapshotMarketValue
      end
    end

    it "returns single SnapshotMarketValueRow for single symbol" do
      with_mock_connection(market_value_response) do |conn|
        result = ThetaData::REST::Stock.snapshot_market_value("AAPL")
        expect(result).to be_a(ThetaData::REST::SnapshotMarketValueRow)
        expect(result.symbol).to be == "AAPL"
        expect(result.market_bid).to be == BigDecimal("154.45")
        expect(result.market_ask).to be == BigDecimal("154.55")
        expect(result.market_price).to be == BigDecimal("154.50")
      end
    end
    ...
    it "returns array for multiple symbols" do
      multi_response = {
        headers: %w[timestamp symbol market_bid market_ask market_price],
        rows: [
          [Time.new(2024, 12, 2), "AAPL", BigDecimal("154.45"), BigDecimal("154.55"), BigDecimal("154.50")],
          [Time.new(2024, 12, 2), "MSFT", BigDecimal("403.00"), BigDecimal("403.10"), BigDecimal("403.05")],
        ],
      }
      with_mock_connection(multi_response) do |conn|
        result = ThetaData::REST::Stock.snapshot_market_value("AAPL", "MSFT")
        expect(result).to be_a(Array)
        expect(result.length).to be == 2
      end
    end
  ...
  with ".history_ohlc" do
    ...
    it "passes start_date/end_date through for multi-day range" do
      with_mock_connection(ohlc_response) do |conn|
        ThetaData::REST::Stock.history_ohlc("AAPL", start_date: Date.new(2024, 12, 1), end_date: Date.new(2024, 12, 31), interval: 60000)
        params = conn.last_call[:request].params
    ...
    it "raises ArgumentError when both date: and start_date:/end_date: are passed" do
      with_mock_connection(ohlc_response) do |_|
        expect do
          ThetaData::REST::Stock.history_ohlc("AAPL", date: Date.new(2024, 12, 1), start_date: Date.new(2024, 12, 1), end_date: Date.new(2024, 12, 31), interval: 60000)
  ...
  with ".history_trade" do
    ...
    it "passes start_date/end_date through for multi-day range" do
      with_mock_connection(trade_response) do |conn|
        ThetaData::REST::Stock.history_trade("AAPL", start_date: Date.new(2024, 12, 1), end_date: Date.new(2024, 12, 31))
        params = conn.last_call[:request].params
        expect(params.start_date).to be == "2024-12-01"
        expect(params.end_date).to be == "2024-12-31"
      end
  ...
  with ".history_quote" do
    ...
    it "passes start_date/end_date through for multi-day range" do
      with_mock_connection(quote_response) do |conn|
        ThetaData::REST::Stock.history_quote("AAPL", start_date: Date.new(2024, 12, 1), end_date: Date.new(2024, 12, 31), interval: 60000)
        params = conn.last_call[:request].params
        expect(params.start_date).to be == "2024-12-01"
        expect(params.end_date).to be == "2024-12-31"
      end
  ...
  with ".history_trade_quote" do
    ...
    it "passes start_date/end_date through for multi-day range" do
      with_mock_connection(trade_quote_response) do |conn|
        ThetaData::REST::Stock.history_trade_quote("AAPL", start_date: Date.new(2024, 12, 1), end_date: Date.new(2024, 12, 31))
        params = conn.last_call[:request].params
        expect(params.start_date).to be == "2024-12-01"
        expect(params.end_date).to be == "2024-12-31"
      end

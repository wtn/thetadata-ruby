...
describe ThetaData::Streaming::StreamHelpers do
  ...
  with "#stream_events" do
    ...
    it "yields nothing when no events match" do
      client = create_mock_client_with_events([
        { type: :ping },
        { type: :metadata, subscriptions: "STOCK.FREE" },
      ])
      ...
      events = []
      helper.stream_events(client, :trade) {|e| events << e }

      expect(events.length).to be == 0
    end
  end

  with "#wait_for_subscription edge cases" do
    it "skips req_response for different request_id" do
      client = create_mock_client_with_events([
        { type: :req_response, request_id: 99, response_code: 0 },
        { type: :req_response, request_id: 1, response_code: 0 },
      ])
  ...
  with "#wait_for_subscriptions edge cases" do
    it "handles interleaved non-matching events" do
      client = create_mock_client_with_events([
        { type: :ping },
        { type: :req_response, request_id: 1, response_code: 0 },
        { type: :ping },
      ...
      Sync do
        helper.wait_for_subscriptions(client, { 1 => "SPX", 2 => "VIX" })
      ensure
        client.close
      end
    end

    it "uses custom error_formatter in error message" do
      client = create_mock_client_with_events([
        { type: :req_response, request_id: 1, response_code: 3 },  # invalid_perms
      ])

      Sync do
        expect {
          helper.wait_for_subscriptions(client, { 1 => "SPX" }) {|id| "INDEX:#{id}" }
        }.to raise_exception(ThetaData::SubscriptionError, message: be(:include?, "INDEX:1"))
      ensure
        client.close
      end
    end

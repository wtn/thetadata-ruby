  ...
  module Streaming
    class Contract
      ...
      def self.from_bytes(data)
        total_length = data.getbyte(0)
        root_length = data.getbyte(1)
        root = data[2, root_length]
        sec_type = data.getbyte(2 + root_length)

        if total_length > 3 + root_length
          # Option contract: sec_type, expiration(u32), is_call(u32), strike(u32)
          expiration, is_call_int, strike = data[3 + root_length, 12].unpack("NCN")
          new(root: root, sec_type: sec_type, expiration: expiration, strike: strike, is_call: is_call_int == 1)
        else
          new(root: root, sec_type: sec_type)
        end
      end

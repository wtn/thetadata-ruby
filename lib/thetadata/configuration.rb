module ThetaData
  class Configuration
    ...
    def initialize
      if (path = ENV["THETADATA_CREDENTIALS_FILE"])
    ...
    def env=(value)
      ...
      servers = SERVERS[@env]
      if servers.nil?
        warn "ThetaData: unknown env #{@env.inspect} -- falling back to :production. Known envs: #{SERVERS.keys.inspect}"
        @env = :production
    ...
    # Reads email (line 1) and password (line 2) from the given file. Mirrors
    # the format used by the Python client and the v3 terminal creds.txt.
    def load_credentials_file(path)
      lines = File.readlines(path)
      @email = lines[0].strip if lines[0]
      @password = lines[1].strip if lines[1]
    end
